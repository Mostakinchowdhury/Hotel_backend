...
def send_otp_via_email(user):
    ...
    send_mail(subject=subject,message=message, html_message=generate_html_otp_message(otp), from_email=settings.DEFAULT_FROM_EMAIL, recipient_list=[user.email])
...
def generate_msg_html(instance):
    ...
    <body>
        <div class="container">
            <h1>Welcome, {instance.username} - {instance.email}!</h1>
...
def send_welcome_email(instance):
    ...
    send_mail(
        ...
        message=f"Welcome to {instance.username} -> {instance.email}!",
